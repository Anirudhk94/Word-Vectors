...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  for _ in range(batch_size // num_skips) :
    ...
    right   = context + 1
    left    = context - 1
    for _ in range(num_skips // 2) :
      batch[batch_index]  = buffer[context]
      labels[batch_index] = buffer[right]
      batch_index += 1
      batch[batch_index]  = buffer[context]
      labels[batch_index] = buffer[left]
      batch_index += 1
      left  -= 1
      right += 1
    buffer.append(data[data_index])
...
if __name__ == '__main__':
  ...
  # batch, labels = generate_batch(data, batch_size=128, num_skips=8, skip_window=4)
  # for i in range(128):
